...
public class DataNode extends Configured 
  ...
  public PeerDataNode peerDatanode;
  ...
  void startDataNode(Configuration conf, 
    ...
    //init P2P peer server
    if(conf.getBoolean("dfs.p2p.enabled",false)) {
	    LOG.info("Initializing and starting the P2P peer datanode");
	    initPeerDataNode(conf);
	    this.peerDatanode.start();
	    this.peerDatanode.openP2PSocket();
    }
    
    // Datanode is completely started at this point
  }

  /**
   * Start the P2P peer server for peer-to-peer connectivity
   * thourgh P2P sockets
   */
  private void initPeerDataNode(Configuration conf) throws IOException {
	  this.peerDatanode = new PeerDataNode(conf);
	  this.peerDatanode.init();
} 
  ...
  public void shutdown() {
    ...
    if (peerDatanode != null) {
    	LOG.info("Shutting down the P2P datanode peer");
    	peerDatanode.shutdown();
    }
