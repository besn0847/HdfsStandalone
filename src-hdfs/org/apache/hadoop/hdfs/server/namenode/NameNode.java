...
import org.apache.hadoop.fs.webdav.WebdavServer;
import org.apache.hadoop.fs.webdav.WebdavServlet;

import com.agorasoft.ubeeko.p2pnamenode.RendezVousNameNode;
...
public class NameNode implements ClientProtocol, DatanodeProtocol,
  ...
  /** P2P Rendez Vous server **/
  private RendezVousNameNode rendezVousServer;
  ...
  private void initialize(Configuration conf) throws IOException, Exception {
    ...
    LOG.info("P2P Rendez-Vous server starting");
    startRendezVousServer(conf);
    this.rendezVousServer.start();
    
    LOG.info("Webdav server starting");
    startWebdavServer(conf);
...
private void startRendezVousServer(Configuration conf) throws IOException {
	  this.rendezVousServer = new RendezVousNameNode(conf);
	  this.rendezVousServer.init();
}
  
 private void startWebdavServer(Configuration conf) throws Exception {
	  String webdavAddr = 
	      NetUtils.getServerAddress(conf, "dfs.webdav.bindAddress", 
	                                "dfs.webdav.port", "dfs.webdav.address");
	    InetSocketAddress webdavSocAddr = NetUtils.createSocketAddr(webdavAddr);
	  
	    String webdavHost = webdavSocAddr.getHostName();
	    int webdavPort = webdavSocAddr.getPort();
	        
	    this.webdavServer = new WebdavServer(webdavHost, webdavPort);
	    LOG.info("WebDAV server up at: " + webdavHost + ":" + webdavPort);
  }
